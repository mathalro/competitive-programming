...
int queryY(int nox, int noy, int ly, int ry, int lyq, int ryq) {
	if (ly > ry) return 0;
	if (ly == lyq && ry == ryq) return st[nox][noy];
	int mid = (ly+ry)/2;
	return queryY(nox, 2*noy, ly, mid, lyq, min(mid, ryq)) +
					queryY(nox, 2*noy+1, mid+1, ry, max(mid+1, lyq), ryq);
	
}

int queryX(int no, int lx, int rx, int lxq, int rxq, int lyq, int ryq) {
	if (lxq > rxq) return 0;
	if (lx == lxq && rx == rxq) return queryY(no, 1, 1, Y, lyq, ryq);
	int mid = (lx+rx)/2;
	return queryX(2*no, lx, mid, lxq, min(mid, rxq), lyq, ryq) +
					queryX(2*no+1, mid+1, rx, max(mid+1, lxq), rxq, lyq, ryq);
}
